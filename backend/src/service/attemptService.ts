...
export const attemptService = {
  async listAvailableExams(studentId: number) {
    return examRepository.listAvailableForStudent(studentId);
  },
  ...
  async getExamForStudent(studentId: number, examId: number) {
    ...
    if (now < new Date(exam.start_date) || now > new Date(exam.end_date)) {
      throw new ApiError(409, "Cet examen n'est pas dans sa fenêtre de disponibilité");
    }

    const existing = await attemptRepository.findByStudentAndExam(studentId, examId);
    if (existing) throw new ApiError(409, 'Vous avez déjà passé cet examen');
  ...
  async submit(studentId: number, examId: number, answers: SubmittedAnswer[]) {
    ...
    try {
      ...
      for (const q of questions) {
        ...
        const choiceId = given?.choiceId ?? null;
        ...
        if (choiceId != null) {
          ...
          pointsAwarded = choice.is_correct ? Number(q.points) : 0;
        }
      ...
      return { score: totalScore, submittedAt: new Date().toISOString(), detail };
    } catch (err) {
